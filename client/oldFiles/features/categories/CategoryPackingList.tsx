...
export default function CategoryPackingList({
  categoryId,
}: CategoryPackingListProps) {
  const [categoryList, setCategoryList] =
    useState<CategoryListType>(defaultCategoryList);
  ...
  const getItems = useCallback(async () => {
    try {
      const responses = await Promise.all(
        categoryList.items.map((itemId: string) => getItem(itemId))
      );
              ...
              <div className="basis-2/12">
                <div className="dropdown dropdown-left">
                  ...
                  >
                    ...
                    <li>
                      <button className="btn btn-ghost">Delete Item</button>
                    </li>
                  </ul>
                </div>
              </div>
  ...
  return (
    <div
      ...
      className="carousel-item flex flex-col flex-wrap ml-10"
    >
