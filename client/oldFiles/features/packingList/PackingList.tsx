...
export default function PackingList(props: PackingListProps) {
  ...
  const getCategoryData = useCallback(async () => {
    try {
      const responses = await Promise.all(
        categoryIds.map((itemId: string) => getCategoryList(itemId))
      );
      const data: CategoryListType[] = await Promise.all(
  ...
  const categoryPackingLists = categoryIds.map((id, index) => {
    return (
      <CategoryPackingList key={index} categoryId={id}></CategoryPackingList>
    );
  });
  ...
  return (
    <div className="">
      <ul className="menu bg-base-200 rounded-box sticky top-0 float-left w-3/12">
        <li key="title" className="menu-title uppercase">
          Categories
        </li>
        {categoryList}
        <li key={"addCategory"}>
