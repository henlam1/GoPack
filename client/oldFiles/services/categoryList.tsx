...
export async function getCategoryList(id: string) {
  try {
    ...
    console.error(
      "A problem occured with retrieving category list " + id,
      error
    );
  }
}
...
export async function addToCategoryList(
  ...
  try {
    ...
    console.error(
      `A problem occured with adding item ${categoryListId} to category list ${itemId}`,
      error
    );
  }
}
