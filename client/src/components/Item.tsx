...
export default function Item({ _id, packed, name, quantity, category}: IItem) {
  ...
  return (
    <>
      ...
      { !categoryContext.editing && (
      <>
        <span className="input input-ghost cursor-pointer" onClick={() => handleEditClick(categoryContext)}>{item.name}</span>
        <span onClick={() => handleEditClick(categoryContext)} className="input input-ghost cursor-pointer">{item.quantity}</span>
        <input
          type="checkbox"
          name="packed"
          checked={item.packed}
