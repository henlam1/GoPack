...
export const UserRegisterSchema = z.object({
  ...
  password: z
    .string()
    .regex(/^(?=.*[A-Za-z])(?=.*\d)[A-Za-z\d]{8,30}$/, {message: "Password needs to be between 8 and 30 characters and have numbers and letters"}),
  email: z
    .string()
    .regex(/^[\w-\.]+@([\w-]+\.)+[\w-]{2,4}$/, {message: "Not a valid email"}),
});
